...
class PostUpdateView(LoginRequiredMixin,View):
    ...
    def post(self , request , *args, **kwargs):
        ...
        if form.is_valid():
            ...
            new_post.save()
            messages.success(request , 'your updated successfully' , 'success')
...
class PostCreateView (LoginRequiredMixin ,View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            new_post.save()
            messages.success(request , 'your create post is successfully' , 'success')
            return redirect('home:post_detail' , new_post.id , new_post.slug)


class PostAddReplyView(LoginRequiredMixin , View):
    form_class = CommentReplyForm
    def post(self , requset , post_id  , comment_id ):
        post =  get_object_or_404(Post , id = post_id)
        comment = get_object_or_404(Comment , id = comment_id )
        form = self.form_class(requset.POST)
        if form.is_valid():
            reply = form.save   (commit=False)
            reply.user = requset.user
            reply.post = post
            reply.reply = comment
            reply.is_reply = True
            reply.save()
            messages.success(requset , "your reply is successfully " , 'success')
        return redirect('home:post_detail' , post.id , post.slug)
